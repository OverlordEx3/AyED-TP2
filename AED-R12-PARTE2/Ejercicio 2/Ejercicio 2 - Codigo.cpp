...
void insertarPrimero(nodo* &listaGoles, tipoGol regGol) { // Crea nuevo nodo con la informacion de regGol y lo inserta en la primera posicion de listaGoles
	nodo* nuevo = new nodo();
	strcpy(nuevo->jugador, regGol.nombreDelJugador);
...
void generarMatriz(nodo* (&matrizGoles)[33][8]) {
	FILE* G = fopen("archivogol.dat", "rb");
	tipoGol regGol;
	...
	while (!feof(G)) {
		...
		posicionBuscada = buscarJugador(matrizGoles[regGol.codigoDeEquipo][codigoFase], regGol.nombreDelJugador); // Verifico si regGol.nombreJugador existe en matrizGoles[regGol.codigoEquipo][codigoFase]
		if (posicionBuscada == NULL) { // Si no existe, creo nuevo nodo e inserto en el primer lugar
			insertarPrimero(matrizGoles[regGol.codigoDeEquipo][codigoFase], regGol);
		}
...
void golesPorEquipo(nodo* matrizGoles[33][8]) { // Crea GOLES_EQUIPO.DAT e imprime la cantidad de goles por equipo
	FILE* G = fopen("goles_equipo.dat", "wb");
	...
	for (i = 1; i < 33; i++) {
		...
		strcpy(regGolesEquipo.nombreEquipo, vectorEquipos[i]);
		regGolesEquipo.goles = cantidadGoles;
		fwrite(&regGolesEquipo, sizeof(tipoGolesEquipo), 1, G);
